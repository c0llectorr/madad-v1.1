...
export function PillButton({
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        s2.pill,
        { backgroundColor: filled ? C.primary : C.surfaceLowest },
        !filled && { borderWidth: 1, borderColor: C.outlineVariant },
        pressed && { opacity: 0.85 },
      ]}
    >
      {icon ? (
        <Text style={{ color: filled ? C.onPrimary : C.primary, fontSize: 16 }}>
          {icon}{" "}
        </Text>
      ) : null}
      <Text style={[T.labelLg, { color: filled ? C.onPrimary : C.primary }]}>
        {title}
      </Text>
    </Pressable>
  );
