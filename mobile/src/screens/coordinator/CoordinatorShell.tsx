...
const SEV_API: Record<string, string> = {
	Low: "low",
	Medium: "medium",
	High: "high",
	Critical: "critical",
...
export default function CoordinatorShell({
	...
	const [planAllocations, setPlanAllocations] = useState<Allocation[] | null>(
		null,
	);
	const [planningReportId, setPlanningReportId] = useState<number | null>(
		null,
	);
	...
	const generatePlanFor = async (site: Site, reportId: number) => {
		...
		try {
			const res = await api<{ allocations: Allocation[] }>(
				"/plan/generate",
	...
	const load = useCallback(() => {
		...
		api<Site[]>(`/sites?center_id=${centerId}`)
			.then(setSites)
			.catch(() => {});
		api<Depot[]>(`/depots?center_id=${centerId}`)
			.then(setDepots)
			.catch(() => {});
		api<Damage[]>(`/roads/damaged?center_id=${centerId}`)
			.then(setDamaged)
			.catch(() => {});
		api<ReportRow[]>(`/reports?center_id=${centerId}`)
			.then(setReports)
			.catch(() => {});
		api<DispatchRow[]>(`/dispatch?center_id=${centerId}`)
			.then(setDispatches)
			.catch(() => {});
		api<CenterRow[]>("/centers")
			.then(setCenters)
			.catch(() => {});
	}, [centerId]);
		...
		return (
			<View style={{ flex: 1, backgroundColor: C.background }}>
				<AppBar
					...
					right={
						<Text
							style={[T.labelLg, { color: C.primary }]}
							onPress={onLogout}
						>
							Logout
						</Text>
				...
				<View style={{ padding: 16 }}>
					<Text style={[T.bodyLg, { color: C.onSurfaceVariant }]}>
						No center assigned to this account.
					</Text>
				</View>
			</View>
		);
			...
			<View style={hs.header}>
				...
				<View style={{ flex: 1, marginLeft: 10 }}>
					...
					<Text style={[T.titleLg, { color: C.onSurface }]}>
						{session.center_name ?? `Center #${centerId}`}
					</Text>
				</View>
					...
					<HomeTab
						...
						onDispatch={(alloc) =>
							setSub({
								name: "plan",
								alloc,
								site: sites.find((s) => s.id === alloc.site_id),
							})
						}
						onAssignSite={(site) =>
							setSub({ name: "assignSite", site })
						}
					/>
				)}
				{tab === "reports" && (
...
function ReportsTab({
	...
	const extract = async (id: number) => {
		...
		setExtractingId(id);
		try {
			const res = await api<any>(`/reports/${id}/extract`, {
				method: "POST",
			});
			setReview(res);
			...
			setRevLat(
				res.extracted.lat != null ? String(res.extracted.lat) : "",
			);
			setRevLng(
				res.extracted.lng != null ? String(res.extracted.lng) : "",
			);
		} catch (e: any) {
			setErr(e.message);
		} finally {
	...
	const reviewSubmit = async (status: "confirmed" | "rejected") => {
		...
		try {
			...
			await api(`/reports/${review.report_id}`, {
				method: "PATCH",
				body,
			});
			setReview(null);
	...
	return (
		...
		>
			<SectionTitle
				title="Field Reports"
				sub={`${reports.length} total · ${reports.filter((r) => r.status === "pending_extraction").length} awaiting extraction`}
			/>
			<Err msg={err} />
			...
			{review && (
				<Card barColor={C.primary}>
					...
					>
						Geocode:{" "}
						{review.geocode_status === "matched"
							? "✅ matched"
							: "⚠️ unmatched — enter coordinates"}
					</Text>
					<Text
						style={[
							T.bodyMd,
					...
					<Text style={[T.bodyMd, { color: C.onSurfaceVariant }]}>
						Flags:{" "}
						{review.extracted.urgency_flags?.join(", ") || "—"}
					</Text>
					<View style={{ marginTop: 8 }} />
					<Field
						label="Location name"
						value={revLoc}
						onChangeText={setRevLoc}
					/>
					...
					<View style={{ flexDirection: "row" }}>
						<View style={{ flex: 1, marginRight: 8 }}>
							<Button
								title="Confirm"
								kind="tertiary"
								onPress={() => reviewSubmit("confirmed")}
							/>
						</View>
						<View style={{ flex: 1 }}>
							<Button
				...
				>
					...
					{!r.raw_text && r.structured_fields && (
						<View style={{ marginTop: 6 }}>
							<Text style={[T.bodyMd, { color: C.onSurface }]}>
								{r.structured_fields.location_name}
							</Text>
							{r.structured_fields.severity && (
								...
								>
									Severity:{" "}
									{r.structured_fields.severity.toUpperCase()}
									{r.structured_fields.headcount
										? `  ·  ~${r.structured_fields.headcount} people`
										: ""}
								</Text>
							)}
							{r.structured_fields.needs.length > 0 && (
								...
								>
									Needs:{" "}
									{r.structured_fields.needs.join(", ")}
								</Text>
							)}
						</View>
					)}
					{r.status === "pending_extraction" && r.raw_text && (
						<View style={{ marginTop: 8 }}>
							<Button
								title={
									extractingId === r.report_id
										? "Extracting… AI is reading the report"
										: "Run AI Extraction"
								}
								kind="outlined"
							...
							{extractingId === r.report_id && (
								...
								>
									This usually takes a few seconds — please
									wait.
								</Text>
							)}
						</View>
								...
								<View style={{ marginTop: 8 }}>
									<Button
										...
										onPress={() => {
											if (!site) {
												setErr(
													"No site record found for this report yet — confirm it first.",
												);
												return;
											}
									...
									{site && (
										<Button
											title={
												planningReportId === r.report_id
													? "Generating plan…"
													: "Generate Plan for this Report"
											}
											kind="tertiary"
											icon={
												planningReportId === r.report_id
			...
			{reports.length === 0 && (
				<Card>
					...
					>
						No reports yet. Use the + button to file your first
						field report.
					</Text>
				</Card>
			)}
...
function MapTab({
	...
	const [redMark, setRedMark] = useState<{ lat: number; lng: number } | null>(
		null,
	);
	...
	const stage: "idle" | "searching" | "confirming" | "saving" = !flagging
		? "idle"
		: saving
			? "saving"
			: redMark
	...
	const submitReport = async () => {
		...
		try {
			...
			showToast(
				"ok",
				"✓ Road blockage reported and saved to the server.",
			);
		} catch (e: any) {
			setSaving(false);
	...
	const affectedSites = useMemo(
		() => sites.filter((s) => s.lat !== 0 || s.lng !== 0),
		[sites],
	);
	const floodHull = useMemo(
		() =>
			convexHull(affectedSites.map((s) => ({ lat: s.lat, lng: s.lng }))),
		[affectedSites],
	);
	...
	const affectedPeople = affectedSites.reduce(
		(sum, s) => sum + (s.estimated_population || 0),
		0,
	);
	const criticalCount = affectedSites.filter(
		...
		() => [
			...
			...(layers.flood
				? affectedSites.map((site) => ({
						...
						snippet:
							`~${site.estimated_population} people` +
							(site.severity
			...
			...(layers.centers
				? centers.map((cn) => ({
						...
						lng: cn.lng,
						title: cn.name,
						snippet: `Support Center · ${cn.code} · ${cn.region ?? ""}`,
						color: C.secondary,
						label: "C",
					}))
				: []),
			...
			...(layers.damage
				? damaged.map((dg) => ({
						...
						snippet:
							dg.reason ||
							"Severe flooding / damage reported here",
						color: C.critical,
						label: "!",
					}))
				: []),
				...
				? [
						{
							...
							snippet:
								'Tap "Yes, this road" to mark it, or tap the map for a precise point',
							color: C.warning,
						},
					]
				: []),
				...
				? [
						{
							...
							snippet: saving
								? "Reporting…"
								: 'Tap "Confirm & Report" to submit, or "Clear mark" to redo',
							color: C.critical,
						},
					]
				: []),
	...
	const polylines: LeafPolyline[] = useMemo(
		() => [
			...(layers.routes
				? dispatches
						.filter((d) => d.route_geojson?.coordinates?.length > 1)
						.map((d) => ({
							id: `r${d.dispatch_id}`,
							coords: d.route_geojson.coordinates.map(
			...
			>
				...
				<Card>
					...
					>
						...
						<Chip
							label="Routes"
							selected={layers.routes}
							color={C.primaryFixedDim}
							onPress={() =>
								setLayers((l) => ({ ...l, routes: !l.routes }))
							}
						/>
					</View>
				</Card>

				{/* Flood impact summary — scroll down to see details when Flood Zone layer is active */}
				{layers.flood && affectedSites.length > 0 && (
					<Card barColor={C.primary}>
						...
						>
							...
							<StatusChip
								label={`${affectedSites.length} sites affected`}
								tone="info"
							/>
						</View>
						<Text
						...
						>
							~{affectedPeople.toLocaleString()} people inside the
							affected region
							{criticalCount > 0
								? ` · ${criticalCount} high/critical site${criticalCount > 1 ? "s" : ""}`
								: ""}
						</Text>
						<Text
							style={[
						...
						>
							The shaded area spans every reported site — its
							spread shows the flood's direction; unreported
							settlements on the fringe are the ones to watch
							next.
						</Text>
					</Card>
				)}
				...
				{toast && (
					<View
						style={[
							mt.toast,
							toast.kind === "ok" ? mt.toastOk : mt.toastErr,
						]}
					>
				...
				{stage === "searching" && (
					<Card barColor={C.warning}>
						...
						>
							Search for the blocked road by name, or tap any
							point on the map to drop a red mark.
						</Text>
						...
						{searchPin && (
							<View style={mt.yellowConfirm}>
								<Text style={mt.yellowConfirmTitle}>
									📍 {searchPin.label.split(",")[0]}
								</Text>
								<Text
									style={[
										T.labelSm,
								...
								>
									Is this the road you meant? Tap below to
									mark it, or tap a different point on the
									map.
								</Text>
								<View style={{ flexDirection: "row", gap: 8 }}>
									<View style={{ flex: 1 }}>
										<Button
											title="Yes, mark this road"
											kind="primary"
											onPress={useSearchPin}
										/>
									</View>
									<View style={{ flex: 1 }}>
										<Button
				...
				{stage === "confirming" && (
					<Card barColor={C.critical}>
						...
						>
							The red pin shows where the blockage will be
							reported. If it's wrong, tap "Clear mark" and re-tap
							the correct spot, or search again.
						</Text>
						...
						<View style={mt.coordRow}>
							...
							>
								📍 {redMark!.lat.toFixed(5)},{" "}
								{redMark!.lng.toFixed(5)}
							</Text>
						</View>
				...
				{activeDispatches.length > 0 && (
					<>
						...
						{activeDispatches.map((d) => (
							...
							>
								<Text
									style={[T.titleLg, { color: C.onSurface }]}
								>
								...
								>
									{d.distance_km?.toFixed(1) ?? "—"} km · ETA{" "}
									{d.eta_minutes ?? "—"} min
								</Text>
								<Text
									style={[
...
function AssignCoordinatorList({
	centerId,
	site,
	onAssigned,
}: {
	...
	const assign = async (coordinatorId: number) => {
		...
		try {
			await api(`/sites/${site.id}/assign`, {
				method: "POST",
	...
	if (doneUser) {
		return (
			<Card barColor={C.tertiary}>
				...
				>
					{doneUser} is assigned to {site.location_name}. The convoy
					route from the nearest depot is live — track it on the Map
					tab.
				</Text>
			</Card>
		);
					...
					<View style={{ marginTop: 10 }}>
						<Button
							title={
								busyId === u.user_id
									? "Starting dispatch…"
									: `Start Dispatch with ${u.username}`
							}
							kind="primary"
							icon="➤"
...
function DispatchesTab({
	...
	const ready = sites.filter(
		(s) => s.status === "unserved" || s.status === "planned",
	);
		...
		return (
			<View style={{ flex: 1, backgroundColor: C.background }}>
				...
				>
					...
					<Card
						barColor={
							SEVERITY_BAR[assignFor.severity ?? "low"] ??
							C.primary
						}
					>
						<Text style={[T.titleLg, { color: C.onSurface }]}>
			...
			>
				...
				{ready.length === 0 && (
					<Card>
						...
						>
							No regions waiting — confirmed reports appear here
							when they need a convoy.
						</Text>
					</Card>
				)}
				{ready.map((s) => {
					...
					return (
						<Card
							key={s.id}
							barColor={
								SEVERITY_BAR[s.severity ?? "low"] ?? C.primary
							}
						...
						>
							...
							>
								Est. Pop ~{s.estimated_population} · from{" "}
								{depot ? depot.name : "nearest depot"}
							</Text>
							<Text
								style={[
									T.labelSm,
							...
							>
								Tap to start dispatch & assign a worker ›
							</Text>
						</Card>
					);
				})}
...
function NewReportScreen({
	...
	const [needs, setNeeds] = useState<string[]>(
		edit
			? (edit.site.needs ?? []).map((n: string) => NEED_LABELS[n] ?? n)
			: [],
	);
	const [flags, setFlags] = useState<string[]>(
		edit
			? (edit.site.urgency_flags ?? []).map(
					(f: string) => FLAG_LABELS[f] ?? f,
				)
			: [],
	);
	...
	return (
		<View style={{ flex: 1, backgroundColor: C.background }}>
			...
			>
				<SectionTitle
					title={
						edit ? `Edit Report #${edit.report_id}` : "New Report"
					}
					sub={
						edit
				...
				{mode === "text" ? (
					<Card barColor={C.primary}>
						...
						>
							Describe the situation — our AI will extract the key
							metrics (location, headcount, needs, urgency).
						</Text>
					...
					<>
						<Card barColor={C.primary}>
							...
							<Field
								...
								placeholder="Location Name (e.g., Jampur)"
							/>
							<View style={{ flexDirection: "row" }}>
								<View style={{ flex: 1, marginRight: 8 }}>
									<Field
...
function PlanResourcesScreen({
	...
	const [qty, setQty] = useState<Record<string, number>>(
		Object.fromEntries(
			alloc.resources.map((r) => [r.resource_type, r.quantity]),
		),
	);
	const [err, setErr] = useState<string | null>(null);
	const [busy, setBusy] = useState(false);
	...
	const dispatch = async () => {
		...
		setErr(null);
		try {
			const resources = Object.entries(qty)
				.filter(([, q]) => q > 0)
				.map(([resource_type, quantity]) => ({
					resource_type,
					quantity,
				}));
			const res = await api<any>("/dispatch", {
				method: "POST",
				body: {
			...
			try {
				const list = await api<CoordinatorRow[]>(
					`/dispatch/available-coordinators?center_id=${centerId}`,
				);
				setCoordinators(
					list.filter(
						(u) => u.is_active && u.user_id !== currentUserId,
					),
				);
			} catch (fe: any) {
	...
	const assign = async (coordinatorId: number, name: string) => {
		...
		setErr(null);
		try {
			await api(`/dispatch/${createdId}/assign`, {
				method: "POST",
				body: { coordinator_id: coordinatorId },
			});
			...
			<View style={{ flex: 1 }}>
				<ScrollView contentContainerStyle={{ padding: 16 }}>
					<SectionTitle
						...
						sub="Adjust the resource quantities before dispatch."
					/>
					<Err msg={err} />

					<Card
						barColor={
							SEVERITY_BAR[site?.severity ?? "low"] ?? C.primary
						}
					>
						<View
							style={{
						...
						>
							Est. population ~{site?.estimated_population ?? "—"}{" "}
							· {alloc.reasoning}
						</Text>
					</Card>
					...
					<Card>
						...
						{Object.keys(qty).length === 0 && (
							...
							>
								No planned resources for this site.
							</Text>
						)}

						{/* Add more resource types from depot stock */}
						{(() => {
							const extra = (depot?.inventory ?? []).filter(
								(i) =>
									!(i.resource_type in qty) && i.quantity > 0,
							);
								...
								<View style={{ marginTop: 8 }}>
									...
									>
										{extra.map((i) => (
											<Chip
												key={i.resource_type}
												label={`＋ ${i.resource_type} (${i.quantity} in stock)`}
												color={C.tertiary}
												onPress={() =>
													setQty((s) => ({
														...s,
						...
						<Card barColor={C.tertiary}>
							{assignedName ? (
								<>
									...
									>
										{assignedName} is on route to{" "}
										{site?.location_name ??
											`site #${alloc.site_id}`}
										. The route is visible to every
										coordinator, and anyone can reroute it
										if the driver hits trouble.
									</Text>
									<View style={{ marginTop: 10 }}>
										<Button
											title="Go to dispatch screen"
											onPress={onDone}
											kind="tertiary"
										/>
									</View>
								...
								<>
									...
									>
										Assign a coordinator of this center to
										the destination — one coordinator per
										destination; only those currently
									...
									{coordinators.length === 0 && (
										...
										>
											No other coordinators are registered
											in this center yet.
										</Text>
									)}
									{coordinators.map((u) => (
									...
									<View style={{ marginTop: 8 }}>
										<Button
											title="Assign later — go to dispatch screen"
											kind="text"
											onPress={onDone}
										/>
									</View>
...
function ActiveRouteScreen({
		...
		try {
			await api("/roads/damage", {
				...
				body: {
					...
					lat: coords.lat,
					lng: coords.lng,
					reason: reason || "Dead end — driver report",
				},
			});
			setFlagged(true);
		} catch (e: any) {
	...
	const reroute = async (silent = false) => {
		...
		try {
			setRoute(
				await api(`/dispatch/${dispatchRow.dispatch_id}/reroute`, {
					method: "POST",
					body: {
	...
	const lastAuto = useRef<{ lat: number; lng: number; at: number } | null>(
		null,
	);
	useEffect(() => {
		if (!coords) return;
	...
	const markDelivered = async () => {
		setErr(null);
		try {
			await api(`/dispatch/${dispatchRow.dispatch_id}/status`, {
				method: "PATCH",
	...
	const dest =
		routeLine?.coordinates?.length > 0
			? {
					lat: routeLine.coordinates[
						routeLine.coordinates.length - 1
					][1],
					lng: routeLine.coordinates[
						routeLine.coordinates.length - 1
					][0],
				}
			: site
	...
	const livePolylines: LeafPolyline[] = useMemo(() => {
		...
		if (routeLine?.coordinates?.length > 1) {
			lines.push({
				id: "route",
				coords: routeLine.coordinates.map((c: number[]) => ({
					lat: c[1],
					lng: c[0],
				})),
				color: C.tertiary,
				...
				>
					...
					{flagged && !route && (
						<Card barColor={C.critical}>
							...
							>
								{reason || "Dead end — driver report"} — damage
								recorded at your GPS position.
							</Text>
					...
					<Card barColor={C.primary}>
						...
						<Text
							style={[
								T.titleLg,
								{ color: C.onSurface, marginTop: 2 },
							]}
						>
							{site?.location_name ??
								`Site #${dispatchRow.site_id}`}
						</Text>
						<View
							style={{
								flexDirection: "row",
						...
						{route && (
							...
							>
								Recalculated · Δ{route.delta_minutes_vs_direct}{" "}
								min vs direct
							</Text>
						)}
						<Text
	...
	return (
		<View style={{ flex: 1, backgroundColor: C.background }}>
			...
			>
				...
				<Card
					barColor={SEVERITY_BAR[site.severity ?? "low"] ?? C.primary}
				>
					<View
						style={{
							flexDirection: "row",
				...
				{result ? (
					<Card barColor={C.tertiary}>
						...
						>
							Dispatch #{result.dispatch_id} is created with a
							damage-aware route to {site.location_name}. ETA{" "}
						...
						>
							...
							<Text style={{ fontWeight: "700" }}>On Route</Text>{" "}
							and they can open the Active Route screen to
							navigate.
						</Text>
						<View style={{ marginTop: 10 }}>
							<Button
					...
					<>
						...
						{coordinators?.length === 0 && (
							<Card>
								...
								>
									No other coordinators are registered in this
									center yet.
								</Text>
							</Card>
						)}
