    ...
    return (
      <View style={{ flex: 1, backgroundColor: C.background }}>
        <AppBar
          title="MADAD"
          right={
            <Text style={[T.labelLg, { color: C.primary }]} onPress={onLogout}>
              Logout
            </Text>
          }
        />
      ...
      <View style={{ flex: 1 }}>
        {tab === "home" && (
          <HomeTab
            ...
            onAssignSite={(site) => setSub({ name: "assignSite", site })}
          />
        )}
        {tab === "reports" && (
          <ReportsTab
          ...
          >
            ...
            <Card barColor={C.secondary}>
              ...
              <Text style={[T.bodyMd, { color: C.onSurfaceVariant }]}>
                Coordinator · {session.center_name ?? `Center #${centerId}`}
              </Text>
            </Card>
    ...
    () => [
      ...(layers.routes
        ? dispatches
            ...
            .map((d) => ({
              id: `r${d.dispatch_id}`,
              coords: d.route_geojson.coordinates.map((c: number[]) => ({
                lat: c[1],
                lng: c[0],
              })),
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <Card>
          ...
          >
            <Chip
              ...
              color="#E65100"
              onPress={() => setLayers((l) => ({ ...l, sites: !l.sites }))}
            />
            <Chip
              label="Depots"
              selected={layers.depots}
              color="#1565C0"
              onPress={() => setLayers((l) => ({ ...l, depots: !l.depots }))}
            />
            <Chip
              label="Road Damage"
              selected={layers.damage}
              color={C.critical}
              onPress={() => setLayers((l) => ({ ...l, damage: !l.damage }))}
            />
            <Chip
              label="Flood Zone"
              selected={layers.flood}
              color={C.primary}
              onPress={() => setLayers((l) => ({ ...l, flood: !l.flood }))}
            />
            <Chip
              label="Centers"
            ...
            <Chip
              ...
              color={C.primaryFixedDim}
              onPress={() => setLayers((l) => ({ ...l, routes: !l.routes }))}
            />
          </View>
        </Card>
        ...
        {layers.flood && affectedSites.length > 0 && (
          <Card barColor={C.primary}>
            ...
            <Text
              style={[T.bodyMd, { color: C.onSurfaceVariant, marginTop: 6 }]}
            >
              ~{affectedPeople.toLocaleString()} people inside the affected
              region
              {criticalCount > 0
            ...
            >
              The shaded area spans every reported site — its spread shows the
        ...
        {toast && (
          <View
            style={[mt.toast, toast.kind === "ok" ? mt.toastOk : mt.toastErr]}
          >
        ...
        {stage === "searching" && (
          <Card barColor={C.warning}>
            ...
            >
              Search for the blocked road by name, or tap any point on the map
              to drop a red mark.
            </Text>
            ...
            {searchPin && (
              <View style={mt.yellowConfirm}>
                ...
                >
                  Is this the road you meant? Tap below to mark it, or tap a
                  different point on the map.
                </Text>
        ...
        {stage === "confirming" && (
          <Card barColor={C.critical}>
            ...
            <Text
              style={[T.bodyMd, { color: C.onSurfaceVariant, marginTop: 4 }]}
            >
              The red pin shows where the blockage will be reported. If it's
              wrong, tap "Clear mark" and re-tap the correct spot, or search
              again.
            </Text>
  ...
  if (doneUser) {
    return (
      <Card barColor={C.tertiary}>
        ...
        <Text style={[T.bodyMd, { color: C.onSurfaceVariant, marginTop: 6 }]}>
          {doneUser} is assigned to {site.location_name}. The convoy route from
          the nearest depot is live — track it on the Map tab.
        </Text>
      </Card>
    );
  }
      ...
      >
        ...
        {ready.length === 0 && (
          <Card>
            ...
            >
              No regions waiting — confirmed reports appear here when they need
              a convoy.
            </Text>
          </Card>
        )}
      ...
      <View style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={{ padding: 16 }}>
          ...
          <Card barColor={SEVERITY_BAR[site?.severity ?? "low"] ?? C.primary}>
            ...
            >
              <Text style={[T.titleLg, { color: C.onSurface, flex: 1 }]}>
                {site?.location_name ?? `Site #${alloc.site_id}`}
              </Text>
              {site?.severity && (
                <StatusChip
          ...
          <Card>
            ...
            {(() => {
              const extra = (depot?.inventory ?? []).filter(
                (i) => !(i.resource_type in qty) && i.quantity > 0,
              );
              ...
              {assignedName ? (
                <>
                  ...
                  >
                    ...
                    {site?.location_name ?? `site #${alloc.site_id}`}. The route
                    is visible to every coordinator, and anyone can reroute it
                ...
                <>
                  ...
                  >
                    Assign a coordinator of this center to the destination — one
                    coordinator per destination; only those currently Available
                    can be selected.
                  </Text>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (status !== "granted") {
          setErr("Location permission denied — enable it in app settings");
          return;
        }
      ...
      <View style={{ flex: 1 }}>
              ...
              ? [
                  {
                    ...
                    lat: dest.lat,
                    lng: dest.lng,
                    title: site?.location_name ?? "Destination",
                    color: C.primary,
                  },
                ]
              : []),
        ...
        >
          ...
          {flagged && !route && (
            <Card barColor={C.critical}>
              ...
              <Text
                style={[T.bodyMd, { color: C.onSurfaceVariant, marginTop: 6 }]}
              >
                {reason || "Dead end — driver report"} — damage recorded at your
                GPS position.
              </Text>
          ...
          <Card barColor={C.primary}>
            ...
            <Text style={[T.titleLg, { color: C.onSurface, marginTop: 2 }]}>
              {site?.location_name ?? `Site #${dispatchRow.site_id}`}
            </Text>
            <View
              style={{
                flexDirection: "row",
            ...
            >
              ...
              {dispatchRow.resources_loaded
                .map((r: any) => `${r.resource_type}×${r.quantity}`)
                .join(", ")}
            </Text>
            <Text
              style={[T.labelSm, { color: C.onSurfaceVariant, marginTop: 4 }]}
