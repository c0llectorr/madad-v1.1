    ...
    () => [
      ...
      ...(layers.depots
        ? depots.map((d) => ({
            ...
            color:
              d.inventory.length === 0 ||
              d.inventory.some((i) => i.quantity < 100)
                ? "#F57F17" // amber — signals low/empty stock
                : "#1565C0", // blue — fully stocked
            label: "D",
          }))
        : []),
    ...
    () => [
      ...(layers.routes
        ? dispatches
            .filter(
              (d) =>
                d.dispatched_by === currentUserId &&
                d.route_geojson?.coordinates?.length > 1,
            )
            .map((d) => ({
              id: `r${d.dispatch_id}`,
        ...
        <Card>
          ...
          >
            ...
            <Chip
              ...
              color={C.secondary}
              onPress={() => {
                setLayers((l) => ({ ...l, centers: !l.centers }));
                mapRef.current?.fitAll();
              }}
            />
            <Chip
