...
export default function CoordinatorNavigator({
  ...
  const generatePlanFor = useCallback(
    async (site: Site, reportId: number) => {
      if (centerId == null) return;
      setPlanningReportId(reportId);
      try {
        const plan = await api<PlanT>("/plan/generate", {
          method: "POST",
          body: { site_id: site.id, center_id: centerId },
        });
  ...
  const load = useCallback(() => {
    ...
    api<CenterRow[]>("/centers")
      .then(setCenters)
      .catch(() => {});
    api<PlanT[]>(`/plans?center_id=${centerId}`)
      .then(setPlans)
      .catch(() => {});
  }, [centerId]);
      ...
      <View style={{ flex: 1 }}>
        ...
        {tab === "reports" && (
          <ReportsPage
            ...
            sites={sites}
            onNewReport={() => setSub({ name: "newReport" })}
            onEditReport={(report_id: number, site: Site) =>
              setSub({ name: "newReport", edit: { report_id, site } })
            }
        ...
        {tab === "dispatches" && (
          <DispatchPage
            ...
            onAssignPlan={(plan: PlanT) =>
              setSub({ name: "assignDriver", plan })
            }
          />
        )}
        {tab === "map" && (
          <MapPage
            ...
            dispatches={dispatches.filter(
              (d) => d.dispatched_by === session.user_id,
            )}
            refresh={refresh}
            currentUserId={session.user_id}
            onOpenRoute={openRoute}
