  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.background }}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {tab === "accounts" && (
          <AccountsTab key2={key} refresh={refresh} go={setView} />
        )}
        {tab === "settings" && (
          <SettingsTab session={session} onLogout={onLogout} />
        )}
      </View>
...
/* ---------------- Resources tab ---------------- */
function ResourcesTab({ centers, key2 }: { centers: Center[]; key2: number }) {
  ...
  const totals = useMemo(() => {
    ...
    Object.values(depotsByCenter)
      .flat()
      .forEach((d) =>
        d.inventory.forEach((i) => {
          map[i.resource_type] = (map[i.resource_type] ?? 0) + i.quantity;
        }),
      );
    ...
    >
      ...
      {centers.length === 0 && (
        <Text style={[T.bodyMd, { color: C.onSurfaceVariant }]}>
          No centers yet — add one to begin.
        </Text>
      )}
        ...
        return (
          <Card key={type} barColor={low ? C.critical : C.tertiary}>
            ...
            >
              ...
              <StatusChip
                label={low ? "Low Stock" : "Adequate"}
                tone={low ? "critical" : "ok"}
              />
            </View>
            ...
            >
              across {Object.values(depotsByCenter).flat().length} depots ·{" "}
    ...
    try {
      await api(`/depots/${depotId}/inventory`, {
        ...
        body: {
          ...
          quantity_delta: sign * parseInt(delta || "0", 10),
        },
      });
      setResType("");
      setDelta("");
  ...
  return (
    ...
    >
      ...
      {depots.map((d) => (
        <Card
          key={d.id}
          barColor={depotId === d.id ? C.tertiary : undefined}
          onPress={() => setDepotId(d.id)}
        >
          <Text style={[T.titleLg, { color: C.onSurface }]}>
            {d.name} {depotId === d.id ? "· selected" : ""}
          </Text>
          {d.inventory.map((i) => (
            <Text
              key={i.resource_type}
      ...
      <Text style={[T.labelLg, { color: C.onSurface, marginTop: 8 }]}>
        New depot
      </Text>
      ...
      <Field
        ...
        placeholder="Resource type (food, water…)"
      />
      <Field
        value={delta}
      ...
      >
        ...
        <Card barColor={C.primary}>
          ...
          <Field
            ...
            placeholder="Unique Code (e.g., C-104)"
  ...
  const submit = async () => {
    ...
    try {
      await api("/accounts/coordinators", {
        method: "POST",
        body: { center_id: centerId, username, password },
      });
      onBack();
    } catch (e: any) {
      setErr(e.message);
      ...
      >
        ...
        <Card barColor={C.primary}>
          ...
          <Text
            style={[
              T.labelLg,
              { color: C.onSurface, marginTop: 4, marginBottom: 8 },
            ]}
          >
            Assigned Center
          </Text>
...
function AdminMapTab({ centers }: { centers: Center[] }) {
  ...
  const floodPolygons: LeafPolygon[] =
    layers.flood && floodHull.length >= 3
      ? [
          {
            id: "floodzone",
            coords: floodHull,
        ...
        <Card>
          ...
          >
            ...
            <Chip
              ...
              color={C.secondary}
              onPress={() => setLayers((l) => ({ ...l, centers: !l.centers }))}
            />
          </View>
        </Card>
